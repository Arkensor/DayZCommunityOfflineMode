class GameMenu extends PopupMenu
{
	...
	string checkboxLayout = "$CurrentDir:missions\\DayZCommunityOfflineMode.Sakhal\\core\\modules\\ComEditor\\gui\\layouts\\CheckboxTemplate.layout";
	...
	override void Init()
	{
		...
		m_gameScriptList.AddItem( "Spawn Nyva",              new Param1< string >( "SpawnHatchback" ), 0 );
		m_gameScriptList.AddItem( "Spawn Volga (White)", 	 new Param1< string >( "SpawnSedanWhite" ), 	   0 );
		m_gameScriptList.AddItem( "Spawn Volga (Wine)", 	 new Param1< string >( "SpawnSedanWine" ), 	   0 );
		m_gameScriptList.AddItem( "Spawn Volga (Black)", 	 new Param1< string >( "SpawnSedanBlack" ), 	   0 );
