class CameraSettings extends PopupMenu 
{
	...
	override void Init()
    {
		...
		CAMERA_ROT = GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.Sakhal\\core\\modules\\CameraTool\\gui\\layouts\\CameraROT.layout" );
		CAMERA_PHI = GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.Sakhal\\core\\modules\\CameraTool\\gui\\layouts\\CameraPHI.layout" );
	}
