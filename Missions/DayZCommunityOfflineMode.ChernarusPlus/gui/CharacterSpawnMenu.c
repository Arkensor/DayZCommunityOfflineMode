class CharacterSpawnMenu extends UIScriptedMenu
{
	...
	void CharacterSpawnMenu(ref SaveModule oSaveModule)
	{
		m_oSaveModule = oSaveModule;
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:\\missions\\DayZCommunityOfflineMode.ChernarusPlus\\gui\\layouts\\CharacterSpawnMenu.layout" );
	...
	override bool OnClick( Widget w, int x, int y, int button )
	{
		if ( w == m_CreateNewButton)
		{
			...
			m_oSaveModule.CreateNew( m_NewCharacterName.GetText() );
		...
		if ( w == m_SpawnButton )
		{
			...
			m_oSaveModule.SavePlayer();
			m_oSaveModule.LoadPlayer(m_sCharacterName, m_sSaveName);
