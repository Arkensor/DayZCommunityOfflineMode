class Fracture: NotifierBase
{
	
	private const float	 	FRACTURED_TRESHOLD	= 0.9;
	private const float	 	CHIPPED_TRESHOLD	= 0.4;
	private const float	 	SPRAINED_TRESHOLD	= 0.15;
		
	void Fracture(NotifiersManager manager)
	{
	...
	override void DisplayBadge()
	{
		...
		float damage = Math.Min(m_Player.GetHealth("RightLeg","Health"),m_Player.GetHealth("LeftLeg","Health"));
